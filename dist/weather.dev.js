...
var req = require("request"); // const url =
...
var geocode = function geocode(address, callback) {
  var urls = "https://api.mapbox.com/geocoding/v5/mapbox.places/" + encodeURIComponent(address) + ".json?access_token=pk.eyJ1IjoiYWJkdWxtdXRsaWIiLCJhIjoiY2wwMmM2aGttMDVvcTNmcWJlZXJ5MjR6OSJ9.TchqzsFIVfNfayYAtDIrnA";
  req({
    url: urls,
    json: true
...
var forecast = function forecast(longitutde, latitude, callback) {
  var url = "http://api.weatherstack.com/current?access_key=ba50f0841e3972445210dce550e03264&query=" + longitutde + "," + latitude + " ";
  req({
    url: url,
    json: true
  }, function (error, response) {
...
if (!location) {
  ...
  geocode(location, function (error, data) {
    if (error) {
      console.log(error);
    } else {
      forecast(data.latitude, data.longitude, function (error, data) {
        console.log(error);
        console.log(data);
      });
    }
